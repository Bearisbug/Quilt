...
import { COLOR_MODES, FONT_SOURCES, fontFamilySchema, fontUrlSchema, paletteSchema, type ColorMode, type DeviceType, type FontSource, type Palette } from '@quilt/core';
...
export function buildMcpServer(user: UserRow): McpServer {
  ...
  // ---- 底层原语（agent 自带模型）----
  server.registerTool('quilt.get_design_contract', { description: 'Machine-checkable design contract: tokens, allowed color classes, component recipes, rules, routes, DESIGN.md.', inputSchema: { projectId: z.string().uuid() } },
