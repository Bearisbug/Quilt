...
import { assembleDocument, buildPrelude, extractBody, injectQids, lintScreenBody, COLOR_CLASS_NAMES, DEVICE_SIZE, type DeviceType, type Tokens, type LintReport } from '@quilt/core';
...
export async function ingestScreen(ownerId: string, projectId: string, input: { name: string; route: string; html?: string; uploadId?: string; screenId?: string; expectedRevisionId?: string; jobId?: string }) {
  ...
  const body = injectQids((raw.includes('<body') ? extractBody(raw) : raw).replace(/\sdata-qid="q\d+"/g, ''));
...
export async function designContract(ownerId: string, projectId: string) {
  ...
  return {
    ...
    tokens: ds.tokens, colorClasses: COLOR_CLASS_NAMES, components: ds.components, designMd: ds.designMd, version: ds.version, assets,
    rules: [
