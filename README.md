...
The server binds to loopback only, so there is no authentication. The 54 tools cover everything the canvas does: projects (`quilt.list_projects`, `create_project`, `get_project`, `update_project`, `delete_project`), reading (`get_outline`, `get_screen`, `get_screenshot`, `get_revision`, `list_revisions`, `list_candidates`, `get_design_contract`, `get_app_map`, `list_assets`, `list_annotations`, `list_messages`, `list_jobs`, `get_job`, `get_job_events`, `list_runners`, `list_design_presets`), writing screens (`validate_screen`, `create_screen`, `update_screen`, `create_upload_url`, `link_screens`, `move_screens`, `edit_element`, `restore_revision`, `adopt_candidate`, `adopt_candidates`, `delete_screen`), shared components (`create_component`, `update_component`, `delete_component`), design system (`update_design_system`, `create_design_preset`, `apply_design_preset`, `delete_design_preset`, `create_asset`, `delete_asset`), model jobs (`generate_screens`, `edit_screens`, `regenerate_subtree`, `edit_component`, `propose_design_system`, `export_prototype`, `get_export`, `cancel_job`, `finish_job`, `create_attachment_upload_url`) and annotations (`update_annotation`, `delete_annotation`, `send_annotations`). Only channel settings (API keys) stay in the settings panel. Resources: `design-md`, `tokens`, `app-map`, `golden`, `screen-html`, `screen-screenshot`, `attachment`.
